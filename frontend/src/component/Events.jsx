import React,{useEffect, useState} from 'react'
import { useSelector } from 'react-redux'
import { useNavigate } from 'react-router-dom'
import axios from 'axios'

// this is the red heard image and white heart image
const redheart='data:image/png;base64,'
const whiteh='data:image/png;base64,'

const Events = ({obj,setchangelike,changelike}) => {
    const navigate=useNavigate()
    const is_login=useSelector((state)=>state.islogin)
    const [token,settoken]=useState(()=>localStorage.getItem("access_token")?JSON.parse(localStorage.getItem("access_token")):null)
    // when i click heart button clicklikebtn call it hit the api if the like exist then unlike it 
    // else creted a like
    const clicklikebtn=(ele)=>{
        if(is_login){
            const headers={
                Authorization: 'Bearer ' + token 
            }
            axios.post(`http://127.0.0.1:8000/api/event/like/${ele.id}/`,{},{headers}).then(()=>{
                setchangelike(!changelike)
            })
        }
        else{
            navigate('/login')
        }      
    }
    
  return (
    <div className="flex flex-col items-center justify-center py-4 w-1/4 h-80 rounded-lg m-3">
            
        <div className="flex items-center mb-2 relative h-3/5">
            <img className='h-full rounded-md ' src={`http://127.0.0.1:8000${is_login==true?'/media/'+obj.image:obj.image}`}/>
            <img className='h-10 w-10 rounded-full cursor-pointer absolute bottom-2 right-2' onClick={()=>clicklikebtn(obj)} src={`${obj.liked_by_user?obj.liked_by_user==true?redheart:whiteh:whiteh}`}/>
        </div>
        
        <h2 className="text-2xl text-center font-bold mb-2">{obj.event_name}</h2>
        <p className="text-gray-500">{obj.date}</p>
        <p className="text-lg font-medium">{obj.location}</p>
        <p className="text-gray-500">From {obj.time}</p>
    </div>
  )
}

export default Events